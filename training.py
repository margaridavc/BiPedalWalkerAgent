...
def train_model(algo, algo_name, policy, n_envs=os.cpu_count()):
    env = make_vec_env(env_id, n_envs=n_envs, vec_env_cls=SubprocVecEnv, wrapper_class=RewardWrapper,
...
def main():
    try:
        ...
        if model_type == "A2C":
            train_model(A2C, model_type, A2C_MlpPolicy)
        elif model_type == "DDPG":
            train_model(DDPG, model_type, DDPG_MlpPolicy, n_envs=1)
        elif model_type == "PPO":
            train_model(PPO, model_type, PPO_MlpPolicy)
        elif model_type == "SAC":
            train_model(SAC, model_type, SAC_MlpPolicy)
        elif model_type == "TD3":
            train_model(TD3, model_type, TD3_MlpPolicy, n_envs=1)
        else:
